...
function RollingTextItem({ item }: RollingTextItemProps) {
  ...
  return (
    ...
    >
      ...
      <span
        className={cn(
          "absolute top-8 right-0 text-xs font-bold uppercase tracking-widest text-neutral-400 transition-opacity duration-300 hidden md:block",
      ...
      >
        <div className="relative h-full w-full">
          <img
            ...
            alt={item.alt}
            className={cn(
              "h-full w-full object-cover grayscale transition-all duration-500 ease-out group-hover:grayscale-0",
              active && "grayscale-0"
            )}
          />
